...
> When [`@adonis-agora/diagnostics-otel`](/docs/diagnostics) is installed, the `@adonis-agora/durable` provider auto-wires this `traceparent` provider for you (read structurally from a global slot, no hard dependency) — so distributed tracing works with zero config in that setup. An explicit `traceparent` in `config/durable.ts` **overrides** that auto-wiring, which is how you bridge a tracer durable does not know about.
