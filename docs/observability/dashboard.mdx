...
1. configure [`dashboardAuth`](/docs/durable/observability/dashboard-auth) — the session guard becomes the gate (the default token guard steps aside);
...
Override `authorize` with your own session/role/IP guard — a denied request gets a `403`: JSON for an API call, the [access-denied page](#the-access-denied-page) for a browser:
