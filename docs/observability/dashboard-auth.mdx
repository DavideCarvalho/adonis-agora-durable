...
- a **page** navigation is redirected `302` to `<path>/login` (carrying a sanitized `returnTo`) when `login` is configured, and answered with a short `401` page reading *"Open this console from your application."* when only `session` is;
