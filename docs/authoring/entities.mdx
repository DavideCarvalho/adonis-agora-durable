...
<Callout type="info">
Under the hood an entity is an ordinary durable run like any other, so everything else in these docs applies to it unchanged: it appears in the [dashboard](/docs/durable/observability/dashboard), it can be inspected and cancelled, and its per-operation checkpoints are the same checkpoints a workflow writes. The five methods above are the supported surface — you never register or start the underlying run yourself.
</Callout>
