...
description: "Undo the side effects of a partially-completed run with per-step compensate callbacks that run in reverse on failure, compensationRetries and compensationTimeoutMs for transient/lost undos, checkpointed (crash-safe) unwinds, compensate:<step> events, and compensating cancellation via engine.cancel(runId, { compensate: true })."
icon: Undo2
---
