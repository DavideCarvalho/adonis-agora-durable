...
An expiry of either window raises the same retryable `RemoteStepTimeout`, so `retries` and `backoff` apply identically. To make either window meaningful, have the handler call [`log.heartbeat(progress?)`](/docs/durable/concepts/workflows-and-steps#reporting-liveness-from-inside-a-long-step) as it works.
