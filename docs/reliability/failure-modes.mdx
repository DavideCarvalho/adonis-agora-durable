...
| You learned about a stuck run from a customer, not an alert | Nothing pages on the stranded signature — `suspended` is not an error | Nothing, until you query for it | Wire [`engine.onStalled`](#the-stalled-run-pager--onstalled--stalledafter) to your pager |
...
2. **`remoteRedispatchMs` / `remoteRedispatchMax` (engine config).** The store-driven net for a no-`timeoutMs` step. Unset by default — opt in explicitly. When set, a dispatched remote step's `pending` checkpoint carries a **lease** in its `wakeAt`: the run suspends *on* that deadline, and only a **lapsed** lease re-dispatches the same `stepId` instead of re-suspending, bumping `attempts` each time, bounded by `remoteRedispatchMax` (default 10) before it gives up and fails the step with a `RemoteStepError` (`code: 'remote_step_lost'`). A worker that is still holding the step **renews the lease with its heartbeats** (throttled to one write per half-window, so a hot beat loop is not an `UPDATE` storm), so a long-but-live step is not re-dispatched just for outliving the window. A replay re-reads a `pending` step from the store before stamping or re-driving it, so a result that landed after the replay took its checkpoint snapshot is replayed, never overwritten by the stale `pending` row. This covers both paths a remote step reaches the broker by — a native `ctx.step` and a polyglot workflow's `call` fan-out (`ctx.gather_calls`) — and a re-drive is marked as such: the `step.started` event carries `redispatched: true` and the checkpoint's own event trail gets a `warn` `step.redispatched` (`step.lost` at the bound), so it reads differently from a failure retry. Two requirements are still non-negotiable: the window **must exceed the longest legitimate step** (or you double-run a slow-but-live worker that isn't beating), and **steps must be idempotent** (re-dispatch can genuinely double-execute a step whose original job merely arrives late).
...
- The checkpoint's `wakeAt` is `NULL` when `remoteRedispatchMs` is unset (nothing is leasing the step); with it set, a `NULL` `wakeAt` on a `pending` remote checkpoint means the engine has not seen this step since it was dispatched, and a lease in the past means the re-drive is due on the next timer tick.
...
- **`wakeForever`** — `suspended` with no wake timer at all, so nothing will ever re-drive it (the `reconcileMs: 0` hazard).
