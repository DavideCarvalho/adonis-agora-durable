  ...
  try {
    ...
    throw new Error(
      [
        ...
        'built for a different Node ABI. Switch to the Node in `.nvmrc`, then:',
        '',
        '  pnpm --filter @adonis-agora/durable rebuild better-sqlite3',
        '',
        'The filter matters: better-sqlite3 is a devDependency of THIS package, not of the',
        'workspace root, so a bare `pnpm rebuild better-sqlite3` at the root matches nothing',
        'and exits 0 without rebuilding anything.',
        '',
